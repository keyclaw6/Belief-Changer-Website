...
const files = {
  '@fontsource/noto-sans-arabic/files/noto-sans-arabic-arabic-400-normal.woff':
    'fonts/noto-sans-arabic-arabic-400-normal.woff',
  ...
  'troika-three-text/dist/troika-three-text.esm.js':
    'troika-three-text.module.js',
  'troika-three-utils/dist/troika-three-utils.esm.js':
    'troika/troika-three-utils.mjs',
  'troika-worker-utils/dist/troika-worker-utils.esm.js':
    'troika/troika-worker-utils.mjs',
  'webgl-sdf-generator/dist/webgl-sdf-generator.mjs':
    'troika/webgl-sdf-generator.mjs',
  'bidi-js/dist/bidi.mjs': 'troika/bidi.mjs',
  '@fontsource/dm-sans/files/dm-sans-latin-500-normal.woff':
...
for (const name of [
  ...
  const pkg = JSON.parse(
    await readFile(resolve(modules, name, 'package.json'), 'utf8'),
  );
  let license = '';
  for (const file of ['LICENSE', 'LICENSE.md', 'OFL.txt']) {
    try {
      license = await readFile(resolve(modules, name, file), 'utf8');
      break;
    } catch {}
  }
...
console.log(
  `Vendored ${Object.keys(files).length} pinned runtime files; no CDN requests required.`,
);
