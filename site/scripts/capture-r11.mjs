/**
 * R11 capture harness (review only, not part of the site bundle).
 * Assumes the prod bridge is already serving on TEST_PORT.
 * Screenshots the isolated /{locale}/reading-room-r11 route per config into
 * r11-captures/, logs pass-criterion metrics per shot + console errors:
 *   - headlineInside / readInside: fraction of headline / Read-link bbox area
 *     inside the room-stage rect (>=0.80 required at 1440 and 390).
 *   - bandGap: stage.top - site-header.bottom in px (<=24 required).
 *   - shelfMin/deskW/ratio: restage checks (shelf >=90 desktop, >=55 mobile;
 *     desk <=2.5x median shelf).
 *   - transferMs: click -> desk aria-label swap time (<=700 required).
 *   - cls: cumulative layout-shift during transfer (<0.05 required).
 *   - overflow: document scrollWidth <= viewport width (no horizontal overflow).
 * R11 adds the one mechanism: shelf-book pickup / desk set-down, 600ms total.
 */
...
async function ready(page, locale) {
  await page.goto(`${base}/${locale}/reading-room-r11`, { waitUntil: 'domcontentloaded' })
...
const file = (name) => new URL(`../../r11-captures/r11-${name}.png`, import.meta.url).pathname
...
for (const s of shots) {
  ...
  console.log(`shot r11-${s.name}.png metrics=${JSON.stringify(m)} pass80/24/no-overflow=${pass} errors=${errors.length}${errors.length ? ' ' + errors.join(' | ') : ''}`)
  await context.close()
}
...
// Rapid clicks: three selections in flight succession drain to the last target.
{
  ...
  // Converge: the queue drains one 600ms transfer at a time toward the last
  // desired target; wait for the desk to land on it, then settle.
  await page.waitForFunction(
