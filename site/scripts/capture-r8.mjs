/**
 * R8 capture harness (review only, not part of the site bundle).
 * Assumes the prod bridge is already serving on TEST_PORT.
 * Screenshots the .reading-room slice per config into r8-captures/,
...
for (const s of shots) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const deskImgs = document.querySelectorAll('.reading-room__desk img').length
  ...
  await page.screenshot({ path: new URL(`../../r8-captures/r8-${s.name}.png`, import.meta.url).pathname })
  console.log(`shot r8-${s.name}.png metrics=${JSON.stringify(metrics)} errors=${errors.length}${errors.length ? ' ' + errors.join(' | ') : ''}`)
  await context.close()
}
...
// Mechanism proof: selecting another shelf book swaps the desk book once.
{
  ...
  await page.screenshot({ path: new URL('../../r8-captures/r8-desktop-light-selected.png', import.meta.url).pathname })
  const audit = await section.evaluate(() => {
    ...
    const deskLabel = document.querySelector('.reading-room__desk')?.getAttribute('aria-label')
