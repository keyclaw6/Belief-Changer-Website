/**
 * R4 capture harness (review only, not part of the site bundle).
...
for (const s of shots) {
  ...
  await page.screenshot({ path: new URL(`../../r4-captures/r4-${s.name}.png`, import.meta.url).pathname })
  console.log(`shot r4-${s.name}.png errors=${errors.length}${errors.length ? ' ' + errors.join(' | ') : ''}`)
  await context.close()
}
...
// Mechanism proof: selecting another cover brings it to the table.
{
  ...
  await page.screenshot({ path: new URL('../../r4-captures/r4-desktop-light-selected.png', import.meta.url).pathname })
