...
const roots = [
  [
    'baseline',
    process.env.ORBIT_BASELINE ||
      path.resolve(current, '../../orbit-baseline/site/public'),
  ],
  [process.env.PROOF_NAME || 'checkpoint', path.join(current, 'public')],
];
const servers = [];
...
try {
  for (let j = 0; j < roots.length; j++) {
    const [name, root] = roots[j];
    const server = createServer(async (req, res) => {
      try {
        if (req.url === '/proof.html') {
    ...
    await new Promise((r) => server.listen(3201 + j, '127.0.0.1', r));
    servers.push(server);
    const page = await browser.newPage({
      viewport: { width: 900, height: 650 },
    });
    page.on('pageerror', (e) => console.log(name, e.message));
    await page.goto(`http://127.0.0.1:${3201 + j}/proof.html`);
    await page.waitForFunction(() => window.ready, {}, { timeout: 30000 });
    results[name] = {};
    for (let i = 0; i < 5; i++) {
      const samples = {};
      await page.evaluate((i) => {
        if (!proof.begin(i)) throw Error('begin failed ' + i);
      }, i);
      for (const t of [
        0.35, 0.45, 0.5, 0.55, 0.6, 0.65, 0.75, 0.85, 0.92, 0.97, 1,
      ]) {
        samples[t] = await page.evaluate(
          ({ i, t }) => {
            proof.scrub(t);
            return proof.vertices(i);
          },
          { i, t },
        );
        if (i === 2)
          await page.screenshot({
            path: path.join(output, `${name}-${t}.png`),
            timeout: 60000,
          });
      }
      samples.rest = await page.evaluate((i) => {
        proof.r.endDrag(true);
        proof.tick(1600);
        return proof.vertices(i);
      }, i);
    ...
    for (let i = 4; i >= 0; i--) {
      await page.evaluate((i) => {
        if (!proof.begin(i, -1, 0.94, 0.58)) throw Error('reverse failed');
        proof.scrub(0.03);
      }, i);
      const before = await page.evaluate((i) => proof.vertices(i), i);
      ...
      results[name]['reverse' + i] = { before, rest };
    }
    await page.close();
  }
  const [a, b] = Object.keys(results);
  const max = (v, w) =>
    v.reduce((m, x, i) => Math.max(m, Math.abs(x - w[i])), 0);
  const summary = { baseline: a, current: b, samples: [] };
  for (let i = 0; i < 5; i++) {
    for (const t of [0.65, 0.75, 0.85, 0.92, 0.97, 1, 'rest'])
      summary.samples.push({
        leaf: i,
        t,
        baselineCurrentMax: max(results[a][i][t], results[b][i][t]),
      });
    summary.samples.push(
      {
        leaf: i,
        t: 'landingJumpBaseline',
        max: max(results[a][i][1], results[a][i].rest),
      },
      {
        leaf: i,
        t: 'landingJumpCurrent',
        max: max(results[b][i][1], results[b][i].rest),
      },
    );
  }
  await writeFile(
    path.join(output, `${b}-comparison.json`),
    JSON.stringify(summary, null, 2),
  );
  console.log(JSON.stringify(summary));
  for (const name of [a, b]) {
    await sharp({
      create: { width: 1800, height: 1300, channels: 3, background: '#dddcd4' },
    })
      .composite(
        await Promise.all(
          [0.65, 0.75, 0.85, 0.92, 0.97, 1].map(async (t, i) => ({
            input: await sharp(path.join(output, `${name}-${t}.png`))
              .resize(600, 650)
              .toBuffer(),
            left: (i % 3) * 600,
            top: Math.floor(i / 3) * 650,
          })),
        ),
      )
      .png()
      .toFile(path.join(output, `${name}-contact.png`));
  }
} finally {
  await browser.close();
