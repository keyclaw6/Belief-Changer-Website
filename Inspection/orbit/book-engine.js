...
async function buildCoverTitleGroup(THREE, shared, { title, subtitle, author, ink }) {
  ...
  const mk = (str, pxSize, yCanvas, opts = {}) => {
    ...
    t.font = opts.serif && shared.serifFontUrl ? shared.serifFontUrl : shared.titleFontUrl;
    ...
    t.letterSpacing = opts.tracking != null ? opts.tracking : -0.005;
    t.fillOpacity = opts.opacity != null ? opts.opacity : 1;
    t.sdfGlyphSize = 64;
    t.renderOrder = 6;
    t.userData.isSDFText = true;
    // Gold parity: depth-tested with troika's polygon offset (depthTest=false
    // made titles bleed through the closed book from behind).
    t.depthOffset = -6;
    t.material.depthTest = true;
    // Group-local +Y points toward the cover top; canvas y grows downward, so
    // canvas-top (yCanvas small) must map to plane-local +Y (gold Y(fy) form).
    t.position.z = 0.012;
    t.position.y = ph / 2 - yCanvas * s;
    g.add(t);
    return t;
  };
  // Site canon (BookCover.tsx): Newsreader 400 at 15.5cqw in the upper
  // negative space (padding-top 20%), DM Sans series mark 3.4cqw tracked
  // 0.34em at the foot (padding-bottom 8%). Canvas basis 768×1152.
  mk(title, 768 * 0.155, 1152 * 0.2, { top: true, lineHeight: 1.06, serif: true });
  if (subtitle) mk(subtitle, 30, 1152 * (0.2 + 0.115) + 60, { lineHeight: 1.3 });
  // The series mark already reads BELIEF CHANGER — skip the author line when it
  // would duplicate it (all orbit books carry author = 'Belief Changer').
  if (author && String(author).toUpperCase() !== 'BELIEF CHANGER') {
    mk(String(author).toUpperCase(), 24, 1152 * 0.862);
  }
  mk('BELIEF CHANGER', 768 * 0.034, 1152 * 0.92, { upper: true, tracking: 0.34, opacity: 0.85 });
...
export async function createSharedResources(THREE, opts = {}) {
  ...
  shared.serifFontUrl = opts.serifUrl || null;
...
export async function createReaderBook(THREE, shared, options = {}) {
  ...
  if (titleOverlay) {
    ...
    // Same board transform as the closed book / cover art plane — without it
    // the title group sat unrotated at the hinge and smeared across the scene.
  ...
  if (pageSys.texture) pageSys.texture.anisotropy = Math.max(pageSys.texture.anisotropy || 1, 16);
  ...
  function openCover(open) {
    ...
    // Reduced motion: the cover sweep is a large full-screen move — snap it.
